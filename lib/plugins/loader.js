...
import messageProcess from '../go-cqhttp/api/message.js'
import groupProcess from '../go-cqhttp/api/group.js'
import messageUtil from '../go-cqhttp/messageUtil.js'
/** 全局变量 plugin */
...
class PluginsLoader {
    ...
    for (let plugin of priority) {
      ...
      if (plugin.rule) {
        ...
        for (let v of plugin.rule) {
          ...
          /**  匹配消息或者小程序 */
          const messageOrApplet = e.msg || e.message?.[0]?.data || e.message;
  ...
  async dealMsg(e) {
      ...
      for (let val of e.message) {
        switch (val.type) {
          ...
          case 'at':
            ...
            break
          case 'file':
            e.file = { name: val.data.name, fid: val.data.fid }
            break
        }
      }
    }
    ...
    if (e.message_type == 'group' || e.notice_type == 'group') {
      ...
      let groupInfo = await groupProcess.get_group_info(e.group_id, true)
        let gInfo= groupInfo.data.data
      if (e.group_name || e.group_name == '') e.group_name = gInfo.group_name
  ...
  reply(e) {
    e.reply = async (msg = '', quote = false, data = {}) => {
      ...
      if (Array.isArray(msg) && msg[0].type === 'node') {
        if (e.isGroup) {
          const res = await messageProcess.sendGroupForwardMsg(e.group_id, msg).catch((err) => {
            logger.error(`发送群消息错误:${msg}`)
            logger.warn(err)
          })
          msgRes = res.data

        } else {
          const res = await messageProcess.sendPrivateForwardMsg(e.group_id, msg).catch((err) => {
            logger.error(`发送私聊消息错误:${msg}`)
            logger.warn(err)
          })
          msgRes = res.data
        }
        ...
        if (typeof (msg) === 'string') {
          ...
          cqMsg = messageUtil.toCqcode(msg)
        }

        if (e.isGroup) {
          const res = await messageProcess.sendGroupMsg(cqMsg, e.group_id).catch((err) => {
            logger.error(`发送群消息错误:${msg}`)
            logger.warn(err)
          })
          msgRes = res.data.data
        } else {
          const res = await messageProcess.sendPrivateMsg(cqMsg, e.user_id).catch((err) => {
            logger.error(`发送私聊消息错误:${msg}`)
            logger.warn(err)
          })
          msgRes = res.data.data
      ...
      if (recallMsg > 0 && msgRes?.message_id) {
        setTimeout(() => messageProcess.deleteMsg(msgRes.message_id), recallMsg * 1000)
      }
