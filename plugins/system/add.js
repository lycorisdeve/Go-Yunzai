...
import messageUtil from '../../lib/go-cqhttp/messageUtil.js'
import imageProcess from '../../lib/go-cqhttp/api/image.js'
import groupProcess from '../../lib/go-cqhttp/api/group.js'
...
export class add extends plugin {
  ...
  async getKeyWord() {
    ...
    let tmpStr = await messageUtil.messageChainToStr(this.e.message)
    this.keyWord = tmpStr.trim()
      /** 过滤#添加 */
  ...
  async addContext() {
    ...
    if (message.length == 1 && message[0].type == 'image') {
      let local = await this.saveImg(message[0].data.file, keyWord)
      if (!local) return
  ...
  getRetMsg() {
    ...
    if (retMsg?.addContext?.message) {
      ...
      for (let i in msg) {
        if (msg[i].type == 'text' && msg[i].data.text.includes('添加')) {
          msg[i].text = this.trimAlias(msg[i].data.text)
          msg[i].text = msg[i].data.text.trim().replace(/#|＃|图片|表情|添加|全局/g, '')
          if (!msg[i].data.text) delete msg[i]
          continue
        }
        if (msg[i].type == 'at') {
          if (msg[i].data.qq == Bot.uin) {
            delete msg[i]
            continue
          } else {
            msg[i].data.text = ''
  ...
  async saveImg(url, keyWord) {
    const imgInfo = await imageProcess.get_image(url)
    let groupCfg = cfg.getGroup(this.group_id)
    ...
    const response = await fetch(imgInfo.data.data.url)
  ...
  async getText() {
    ...
    if (msg[0] && msg[0].local) {
      if (fs.existsSync(msg[0].local)) {
        let tmp = segment.image(`file:///${process.cwd()}${msg[0].local}`)
    ...
    if (Array.isArray(msg)) {
      msg.forEach(m => {
        /** 去除回复@@ */
        if (m?.type == 'at') { delete m.data.text }
      })
    }
  ...
  async del() {
    ...
    this.initTextArr()
    let tmpStr = await messageUtil.messageChainToStr(this.e.message)
    let keyWord = tmpStr.replace(/#|＃|图片|表情|删除|全部|全局/g, '')
    ...
    for (let msg of this.e.message) {
      if (msg.type == 'text') {
        msg.data.text = msg.data.text.replace(/#|＃|图片|表情|删除|全部|全局/g, '')

        if (!msg.data.text) continue
      }
      retMsg.push(msg)
    }
    if (num > 0) {
